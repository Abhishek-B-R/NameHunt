...
export async function checkDynadot(
  ...
  try {
    ...
    if (!(await row.isVisible().catch(() => false))) {
      ...
      return { ok: false, domain, error: "Result row not found", rawText: body.slice(0, 900) };
    }

    // Confirm the row domain matches exactly
    const rowDomainText =
      (await row.locator(".search-domain-word").first().innerText().catch(() => "")) || "";
    if (rowDomainText && rowDomainText.trim().toLowerCase() !== domain.toLowerCase()) {
      const exactRow = page
        .locator(`div.result-row:has(.search-domain-word:has-text("${domain}"))`)
        .first();
    ...
    const saysAvailable =
      ...
      /\bis available\b/i.test(rowText);
    ...
    const renewalText =
      (await row.locator(".search-renewal").first().innerText().catch(() => "")) || "";
    const renewalParsed = parsePrice(renewalText);
    const renewalPrice = renewalParsed.amount;

    // Registration price from this row
    let regText =
      (await row.locator(".domain-price .search-price").first().innerText().catch(() => "")) ||
      "";
    ...
    if (registrationPrice === undefined) {
      const prevText =
        (await row.locator(".domain-price .prev-price").first().innerText().catch(() => "")) ||
        "";
    ...
    if (registrationPrice === undefined) {
      ...
      const prices = parseAllPrices(joined).map((p) => p.amount).filter((n): n is number => !!n);
      if (prices.length) {
        const uniq = Array.from(new Set(prices));
        if (renewalPrice !== undefined) {
          const candidates = uniq.filter((a) => Math.abs(a - renewalPrice) > 1e-6);
          registrationPrice = candidates.length ? Math.max(...candidates) : Math.max(...uniq);
        } else {
