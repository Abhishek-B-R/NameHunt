...
export async function checkDomainPorkbun(
  ...
  try {
    ...
    const searchInput = page.locator('input[type="text"], input[placeholder*="domain"], input[name*="search"], #searchInput');
    ...
    const renewMatch = domainSection.match(/renews?\s+at[^$€£₹]*(\$|€|£|₹)\s*([0-9][\d,]*\.?\d*)/i);
